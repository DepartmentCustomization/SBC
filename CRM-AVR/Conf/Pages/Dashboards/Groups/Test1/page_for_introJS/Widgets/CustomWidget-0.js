    ...
    return {
        ...
        customConfig:
                ...
                .menuCategoryItem, .arrow{
                    border-bottom: 1px solid #004479a8;
                    display: flex;
                    flex-direction: row;
                    align-items: center;
                    width: 100%;
        ...
        */
        ...
        createInfo: function() {
            ...
            menu_container.appendChild(i_tabs_container);
            i_tabs_container.addEventListener('click', event => {
                this.showIntroJs();
            });
            document.getElementById('menuCategory_2').setAttribute('data-step', '1');
            document.getElementById('menuCategory_2').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_2').setAttribute('data-intro', 'Перший пункт меню, що відповідає за заявки та зв’язані з ними дані');
            document.getElementById('menuCategory_3').setAttribute('data-step', '2');
            document.getElementById('menuCategory_3').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_3').setAttribute('data-intro', 'Другий пункт меню відповідає за перегляд та пошук виїздів');
            document.getElementById('menuCategory_4').setAttribute('data-step', '3');
            document.getElementById('menuCategory_4').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_4').setAttribute('data-intro', 'Третій пункт меню, що відповідає за підрозділи водоканалу');
            document.getElementById('menuCategory_5').setAttribute('data-step', '4');
            document.getElementById('menuCategory_5').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_5').setAttribute('data-intro', 'Четвертий пункт меню, що відповідає за пошук загальної інформації організації');
            document.getElementById('menuCategory_6').setAttribute('data-step', '5');
            document.getElementById('menuCategory_6').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_6').setAttribute('data-intro', 'П’ятий пункт меню, що відповідає за пошук технічної  інформації організації');
            document.getElementById('menuCategory_7').setAttribute('data-step', '6');
            document.getElementById('menuCategory_7').setAttribute('data-intro-group', 'group_left_menu');
            document.getElementById('menuCategory_7').setAttribute('data-intro', 'Шостий пункт меню, що відповідає за поточний стан заявок');
        },
        showIntroJs: function() {
            introJs().setOption('nextLabel', 'Go Next');
            introJs().start('group_left_menu');
        },
    	...
    	ParentId = data.columns.findIndex(el => el.code.toLowerCase() === 'parentid');
            ...
            for (let i = 0; i < data.rows.length; i++) {
                if(data.rows[i].values[ParentId] == null && data.rows[i].values[isVisible] != true) {
                    ...
                    const menuCategoryItem_icon = this.createElement('div', {className: 'menuCategoryItem_icon material-icons', innerText: String(String(data.rows[i].values[Icon]))});
                    menuCategoryItem_icon.style.color = String(String(data.rows[i].values[IconColor]));
                    const menuCategoryItem_title = this.createElement('div', {className: 'menuCategoryItem_title', innerText: String(String(data.rows[i].values[Name]))});
                    ...
                    const menuCategory = this.createElement('div', {className: 'menuCategory', id: String('menuCategory_' + data.rows[i].values[Id])}, menuCategoryItem, childrensContainer);
        ...
        createMenuElements: function(data) {
            for (let i = 0; i < data.rows.length; i++) {
                if(data.rows[i].values[Type] == 'INPUT' && data.rows[i].values[List] == false && data.rows[i].values[isVisible] != true) {
                    ...
                    if(element == null) {
                        var parentElement = document.getElementById(String('menuItemWrapper_' + data.rows[i].values[ParentId]));
                    }else{
                        var parentElement = element;
                    }
                    let menuItem_title = this.createElement('div', {className: 'menuItem_title', innerText: String(String(data.rows[i].values[Name]))});
                    let menuItem = this.createElement('div', {className: 'menuItem', place: String(String(data.rows[i].values[placeTo])), link: String(String(data.rows[i].values[urlLink])), id: String('menuItem_' + data.rows[i].values[Id])}, menuItem_title);
                    ...
                    let menuItemWrapper = this.createElement('div', {className: 'menuItemWrapper', id: String('menuItemWrapper_' + data.rows[i].values[Id]) }, menuItem, childrensContainer);
                    if(parentElement != null) {
                    ...
                    let menuForm_title = this.createElement('div', {className: 'menuForm_title', innerText: String(String(data.rows[i].values[Name]))});
                    let menuForm = this.createElement('div', {className: 'menuForm', value: true}, menuForm_title, menuForm_arrow);
                    let menuFormWrapper = this.createElement('div', {className: 'menuFormWrapper', id: String('menuFormWrapper_' + data.rows[i].values[Id]) }, menuForm, childrensContainer);
                    if(parentElement != null) {
        ...
        createMenuFormElements: function(data, parentElement, parId, menuFormWrapper) {
            for (let i = 0; i < data.rows.length; i++) {
                if(data.rows[i].values[Type] == 'FORM_INPUT' && parId == data.rows[i].values[ParentId] && data.rows[i].values[isVisible] != true) {
                    let menuFormInput = this.createElement('input', {className: 'menuFormInput', key: String(String(data.rows[i].values[key])), placeholder: String(String(data.rows[i].values[Name])), id: String(String(data.rows[i].values[inputId]))});
                    parentElement.appendChild(menuFormInput);
                }
                if(data.rows[i].values[Type] == 'BTN' && parId == data.rows[i].values[ParentId]) {
                    let menuFormButton = this.createElement('button', {className: 'menuFormButton', url: String(String(data.rows[i].values[urlLink])), innerText: String(String(data.rows[i].values[Name])) , id: String(String(data.rows[i].values[inputId]))});
