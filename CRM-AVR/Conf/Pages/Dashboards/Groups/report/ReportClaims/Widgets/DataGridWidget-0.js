(function () {
  return {
    ...
    init: function() {
        ...
        var sub0 = this.messageService.subscribe('GlobalFilterChanged', this.GlobalFilterChanged_funk, this);
        ...
        */
        var sub11 = this.messageService.subscribe('LoadData', this.LoadData_funk, this);
        this.subscriptions.push(sub11);
        
        var sub12 = this.messageService.subscribe('ExportToExcel', this.exportToExcel, this);
    ...
    GlobalFilterChanged_funk: function(message){
        ...
        //  this.val_chance_filter_Claim_types = this.checkValueMultiList(message.package.value.values[6].value);
         ...
         if (this.val_chance_filter_Claim_types == undefined) {this.val_chance_filter_Claim_types = ''};


         this.val_chance_filter_Contacts = this.checkValueMultiList(message.package.value.values[7].value);
  
        let val = "";
        if (message.package.value.values[8].value == "") {val = "%"} else {val = message.package.value.values[8].value;};
         let valuesList = [];
         valuesList.push(val);
