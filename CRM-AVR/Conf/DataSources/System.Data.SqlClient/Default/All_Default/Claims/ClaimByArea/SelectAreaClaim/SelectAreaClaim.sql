-- DECLARE @Id INT = 9055;
 ...
 DECLARE @CreatorName NVARCHAR(200) = (SELECT 
										...
										WHERE UserID = (SELECT 
														...
														FROM dbo.Claims 
														WHERE Id = @Id));
 DECLARE @HistoryID INT = (SELECT MAX(Id) FROM dbo.Claims_History WHERE Claims_ID = @Id);
...
SELECT 
	  ...
	  IIF(cl.Status_ID = 5,
	   (SELECT 
	    TOP 1 ch.[Date] 
	    FROM dbo.Claims cl
		LEFT JOIN dbo.Claims_History ch ON ch.Claims_ID = cl.Id
	    WHERE cl.id = @Id 
		ORDER BY ch.Id DESC),
		NULL ) AS date_close,
...
FROM dbo.Claims cl 
INNER JOIN dbo.Claim_classes clcl ON clcl.Id = cl.Claim_class_ID
INNER JOIN dbo.Claim_types ct ON ct.Id = cl.Claim_type_ID
INNER JOIN dbo.Organizations o ON o.Id = cl.Response_organization_ID
