...
insert CrossSTR (Name, Streets_1_ID, Streets_2_ID)
	...
	values
		(
			(select Name from Streets where [Street_Id] = @m_houses_id) + '/'+ (select Name from Streets where [Street_Id] = @m_houses2_id)
			,@m_houses_id
			,@m_houses2_id
		)
	...
	select 19, @m_district, Name, isnull(@latitude ,null), isnull(@longitude ,null), @cross_id from CrossSTR where Id = @cross_id
...
select @place_id as Id,
        (select Name from Places where Id = @place_id) as Name,
         19 as type_place, 
        (select Name from Place_types where Id = 19) as type_place_name,
