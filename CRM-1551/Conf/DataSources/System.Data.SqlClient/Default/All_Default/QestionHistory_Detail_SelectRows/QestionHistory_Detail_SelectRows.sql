...
FROM
	...
	LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].[Question_History] AS t2 ON t2.Id = @history_id_old
	LEFT JOIN [dbo].[QuestionTypes] AS qt1 ON qt1.Id = t1.question_type_id
	LEFT JOIN [dbo].[QuestionTypes] AS qt2 ON qt2.Id = t2.question_type_id
WHERE
	t1.Id = @history_id
...
OFFSET @pageOffsetRows ROWS FETCH next @pageLimitRows ROWS ONLY ; ' ;
 ...
 EXEC sp_executesql @Query, N'@history_id INT', @history_id = @history_id;
END
END