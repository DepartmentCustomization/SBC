...
declare @date_from date='2019-09-01',
  ...
  */
  ...
  ltrim((select count(Id) from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite] where [AppealFromSiteResultId]=1 and convert(date, ReceiptDate) between convert(date, @date_from) and convert(date, @date_to))) inf3_1,

  ltrim((select count(Id) from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite] where convert(date,ReceiptDate)=getutcdate() and [AppealFromSiteResultId]=1)) inf3_2,
  ...
  and convert(date, ReceiptDate) between convert(date, @date_from) and convert(date, @date_to))) inf3_3,
