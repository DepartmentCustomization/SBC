...
where [PosibilitiesForRoles].role_id in (SELECT  [role_id] FROM [dbo].[Positions] where programuser_id = @programuser_id)
and RulesforRightsId in (SELECT [id] FROM [dbo].[RulesforRights] where formcode = @form)
and  [PosibilitiesForRoles].colunmcode not in (N'complaintype', N'transitionassignmentstate')
and [PosibilitiesForRoles].can_do = 0
