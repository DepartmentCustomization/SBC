...
isnull([User].[LastName], N'')+N' '+isnull([User].[FirstName], N'')+N' - '+(select ltrim(count(Id)) from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite])+

N' ('+ltrim(convert(numeric(8,2), convert(float,count([AppealsFromSite].Id))/convert(float, (select count(Id) from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite]))*100))+N'%)' leg_name
into #leg
...
group by [AppealsFromSite].EditByUserId, isnull([User].[LastName], N'')+N' '+isnull([User].[FirstName], N'')
