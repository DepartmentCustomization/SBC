...
LEFT JOIN [dbo].Questions Questions
...
SELECT TOP 1
   ...
   ,(SELECT
			current_assignment_consideration_id
		FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].Assignments
		WHERE Id = @Id)
	AS assignmentConsiderations_id
   ,(SELECT
			COUNT(assg.main_executor)
		FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Assignments] assg
		WHERE assg.question_id = Assignments.question_id
		AND assg.main_executor = 1
		AND assg.close_date IS NULL)
	AS is_aktiv_true
   ,(SELECT TOP 1
			control_comment
		FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].AssignmentRevisions AS ar
		JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].AssignmentConsiderations AS ac
			ON ac.Id = ar.assignment_consideration_іd
		WHERE ac.assignment_id = @Id
		ORDER BY ar.Id DESC)
	AS control_comment
	,ISNULL((SELECT TOP 1 ar.rework_counter
	FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Assignments] a
	INNER JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[AssignmentConsiderations] ac ON a.Id=ac.assignment_id
	INNER JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[AssignmentRevisions] ar ON ar.assignment_consideration_іd=ac.Id
	WHERE a.Id=@Id
	ORDER BY ar.Id DESC),0) rework_counter
   ,assC.[transfer_to_organization_id]
   ...
   ,STUFF((SELECT
			...
			END
		FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Assignments] a
		LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Questions] q
			ON a.question_id = q.Id
		LEFT JOIN [dbo].[AnswerTypes] [AnswerTypes]
			ON q.answer_form_id = [AnswerTypes].Id
		WHERE a.Id = [Assignments].Id
   ...
   ,CASE
	...
	,[Questions].[geolocation_lat]
	,[Questions].[geolocation_lon]
FROM [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Assignments] Assignments
LEFT JOIN [dbo].AssignmentTypes aty
	ON aty.Id = Assignments.assignment_type_id
LEFT JOIN [dbo].AssignmentStates ast
	ON ast.Id = Assignments.assignment_state_id
LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].Questions Questions
	ON Questions.Id = Assignments.question_id
LEFT JOIN [dbo].QuestionTypes QuestionTypes
	ON QuestionTypes.Id = Questions.question_type_id
LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].Appeals Appeals
	ON Appeals.Id = Questions.appeal_id
LEFT JOIN [dbo].ReceiptSources ReceiptSources
	ON ReceiptSources.Id = Appeals.receipt_source_id
LEFT JOIN [dbo].Applicants Applicants
...
LEFT JOIN [dbo].ObjectTypes [ObjectTypes] 
	ON ObjectTypes.Id = [Objects].object_type_id
LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].AssignmentConsiderations assC
	ON assC.Id = Assignments.current_assignment_consideration_id
LEFT JOIN [dbo].AssignmentResults assR
	ON assR.Id = Assignments.AssignmentResultsId
LEFT JOIN [dbo].AssignmentResolutions assRn
	ON assRn.Id = Assignments.AssignmentResolutionsId
LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].AssignmentRevisions assRev
	ON assRev.assignment_consideration_іd = assC.Id
LEFT JOIN [dbo].Organizations AS perf
	ON perf.Id = Assignments.executor_organization_id
LEFT JOIN [dbo].Organizations AS responsible
...
SELECT TOP 1
   ...
   ,(SELECT
			current_assignment_consideration_id
		FROM [10.192.200.182].[CRM_1551_Analitics].[dbo].Assignments
		WHERE Id = @Id)
	AS assignmentConsiderations_id
   ,(SELECT
			COUNT(assg.main_executor)
   ...
   ,(SELECT TOP 1
			control_comment
		FROM [10.192.200.182].[CRM_1551_Analitics].[dbo].AssignmentRevisions AS ar
		JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].AssignmentConsiderations AS ac
			ON ac.Id = ar.assignment_consideration_іd
		WHERE ac.assignment_id = @Id
		ORDER BY ar.Id DESC)
	AS control_comment
	,ISNULL((SELECT TOP 1 ar.rework_counter
	FROM [dbo].[Assignments] a
	INNER JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].[AssignmentConsiderations] ac ON a.Id=ac.assignment_id
	INNER JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].[AssignmentRevisions] ar ON ar.assignment_consideration_іd=ac.Id
	WHERE a.Id=@Id
	ORDER BY ar.Id DESC),0) rework_counter
   ,assC.[transfer_to_organization_id]
   ...
   ,STUFF((SELECT
			...
			CASE
				...
				WHEN q.[answer_mail] IS NOT NULL THEN q.[answer_mail]
			END
		FROM [10.192.200.182].[CRM_1551_Analitics].[dbo].[Assignments] a
		LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].[Questions] q
			ON a.question_id = q.Id
		LEFT JOIN [dbo].[AnswerTypes] [AnswerTypes]
			ON q.answer_form_id = [AnswerTypes].Id
		WHERE a.Id = [Assignments].Id
   ...
   ,CASE
	...
	,[Questions].[geolocation_lat]
	,[Questions].[geolocation_lon]
FROM [10.192.200.182].[CRM_1551_Analitics].[dbo].[Assignments] Assignments
LEFT JOIN [dbo].AssignmentTypes aty
	ON aty.Id = Assignments.assignment_type_id
LEFT JOIN [dbo].AssignmentStates ast
	ON ast.Id = Assignments.assignment_state_id
LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].Questions Questions
	ON Questions.Id = Assignments.question_id
LEFT JOIN [dbo].QuestionTypes QuestionTypes
	ON QuestionTypes.Id = Questions.question_type_id
LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].Appeals Appeals
	ON Appeals.Id = Questions.appeal_id
LEFT JOIN [dbo].ReceiptSources ReceiptSources
	ON ReceiptSources.Id = Appeals.receipt_source_id
LEFT JOIN [dbo].Applicants Applicants
...
LEFT JOIN [dbo].ObjectTypes [ObjectTypes] 
	ON ObjectTypes.Id = [Objects].object_type_id
LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].AssignmentConsiderations assC
	ON assC.Id = Assignments.current_assignment_consideration_id
LEFT JOIN [dbo].AssignmentResults assR
	ON assR.Id = Assignments.AssignmentResultsId
LEFT JOIN [dbo].AssignmentResolutions assRn
	ON assRn.Id = Assignments.AssignmentResolutionsId
LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].AssignmentRevisions assRev
	ON assRev.assignment_consideration_іd = assC.Id
LEFT JOIN [dbo].Organizations AS perf
	ON perf.Id = Assignments.executor_organization_id
LEFT JOIN [dbo].Organizations AS responsible
