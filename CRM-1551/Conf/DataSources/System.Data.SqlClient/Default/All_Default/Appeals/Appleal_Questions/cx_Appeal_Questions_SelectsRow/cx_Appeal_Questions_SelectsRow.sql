...
FROM
	...
	LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[Assignments] Assignments ON Assignments.question_id = Questions.Id
	AND Assignments.main_executor = 1 
	...
	LEFT JOIN [DB.UKRODS.CF].[CRM_1551_Analitics].[dbo].[AssignmentResolutions] assRn ON assRn.Id = Assignments.AssignmentResolutionsId
	LEFT JOIN [dbo].[Organizations] perfom ON perfom.Id = Assignments.[executor_organization_id]
	LEFT JOIN [#system_database_name#].[dbo].[User]  [User] ON [Questions].[user_id] = [User].UserId
	--LEFT JOIN [CRM_1551_System].dbo.[User] [User] ON [Questions].[user_id] = [User].UserId
WHERE
...
(
		SELECT
			...
			END
		FROM
			[10.192.200.182].[CRM_1551_Analitics].dbo.Assignments
		WHERE
			question_id = @Id
			AND main_executor = 1
	) AS flag_is_state
...
FROM
	...
	LEFT JOIN [CRM_1551_Analitics].[dbo].[ObjectTypes] ObjectTypes ON ObjectTypes.Id = [Objects].object_type_id
	LEFT JOIN [CRM_1551_Analitics].[dbo].[Districts] Districts ON Districts.Id = [Buildings].district_id
	LEFT JOIN [10.192.200.182].[CRM_1551_Analitics].[dbo].[Assignments] Assignments ON Assignments.question_id = Questions.Id
	AND Assignments.main_executor = 1
	...
	LEFT JOIN [CRM_1551_Analitics].[dbo].[AssignmentResults] assR ON assR.Id = Assignments.AssignmentResultsId
	LEFT JOIN [CRM_1551_Analitics].[dbo].[AssignmentResolutions] assRn ON assRn.Id = Assignments.AssignmentResolutionsId
	LEFT JOIN [dbo].[Organizations] perfom ON perfom.Id = Assignments.[executor_organization_id]
	LEFT JOIN [#system_database_name#].[dbo].[User] [User] ON [Questions].[user_id] = [User].UserId
	--LEFT JOIN [CRM_1551_System].dbo.[User] [User] ON [Questions].[user_id] = [User].UserId
WHERE
