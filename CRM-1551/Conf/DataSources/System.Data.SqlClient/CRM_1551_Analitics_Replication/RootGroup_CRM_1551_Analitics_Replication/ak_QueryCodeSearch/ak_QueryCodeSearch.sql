...
/*
 ...
 declare @transfer_date_to datetime;--='2019-07-05 11:26:37';
  ...
  declare @state_changed_date_from DATETIME;--='2019-03-01 11:04:59';
 declare @state_changed_date_to DATETIME;--='2020-02-07 11:05:07';
...
declare @registration_date_question_fromP nvarchar(200)=
 ...
 then N' and control_date<= '''+format(convert(datetime2, @control_date_to), 'yyyy-MM-dd HH:mm:59')+N'.999'''
  ...
  --чисто для timeliness начало
  DECLARE @comment_timeliness NVARCHAR(4)
  =CASE WHEN CHARINDEX(N'timeliness in (', @param1, 0)>0 THEN N'' ELSE N'--' END;

  DECLARE @param_timeliness NVARCHAR(100)=
CASE WHEN CHARINDEX(N'timeliness in (1, 2)', @param1, 0)>0 THEN N'timeliness_table.min_date is not null'
WHEN CHARINDEX(N'timeliness in (1)', @param1, 0)>0 THEN N'timeliness_table.min_date<=[Assignments].[execution_date]'
WHEN CHARINDEX(N'timeliness in (2)', @param1, 0)>0 THEN N'timeliness_table.min_date>[Assignments].[execution_date]'
ELSE N'1=1' END

--SELECT @param_timeliness 
  -- чисто для timeliness конец
...
declare @n int=1;
  ...
  --опять timeliness начало

  SET @param_new=
CASE WHEN CHARINDEX(N'timeliness in (1, 2)', @param1, 0)>0 THEN REPLACE(@param_new, N'timeliness in (1, 2)', N'1=1')
WHEN CHARINDEX(N'timeliness in (1)', @param1, 0)>0 THEN REPLACE(@param_new, N'timeliness in (1)', N'1=1')
WHEN CHARINDEX(N'timeliness in (2)', @param1, 0)>0 THEN REPLACE(@param_new, N'timeliness in (2)', N'1=1')
ELSE @param_new END

  --опять timeliness конец

  --SET @param_new=REPLACE(@param_new, )
...
--select @param_new, @organizations, @param_timeliness, @comment_timeliness
...
when [Applicants].[birth_date] is null then year(getdate())-[Applicants].birth_year
  ...
  */
  ...
  '+@comment_timeliness+'inner join (select [assignment_id], min([Log_Date]) min_date 
  '+@comment_timeliness+'from [Assignment_History] where assignment_state_id=3 and Log_Activity=N''UPDATE'' 
  '+@comment_timeliness+'group by [assignment_id]) timeliness_table on [Assignments].Id=timeliness_table.[assignment_id]
