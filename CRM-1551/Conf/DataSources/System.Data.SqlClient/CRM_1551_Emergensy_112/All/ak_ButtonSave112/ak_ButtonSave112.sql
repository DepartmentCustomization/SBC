...
--   WHERE s.id IN ('+ISNULL(@service_ids, N'0')+N')';
  ...
  SELECT 
      ...
      ,GETUTCDATE() [edit_date]
  FROM [dbo].[Classes]
  WHERE id IN ('+CASE WHEN @applicant_classes_ids IS NULL OR @applicant_classes_ids=N'' THEN N'0' ELSE @applicant_classes_ids END+N')
  ...
  SELECT 
      ...
      ,GETUTCDATE() [edit_date]
  FROM [dbo].[Classes]
  WHERE id IN ('+CASE WHEN @applicant_classes_ids IS NULL OR @pacient_classes_ids=N'' THEN N'0' ELSE @pacient_classes_ids END+N')';
