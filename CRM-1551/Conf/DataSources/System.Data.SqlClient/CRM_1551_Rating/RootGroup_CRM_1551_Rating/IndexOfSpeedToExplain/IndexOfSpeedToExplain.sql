...
if isnull(@RDAId,0) = 0
begin 
	--IndexOfSpeedToExplain_QType
	if object_id('tempdb..#temp_IndexOfSpeedToExplain_QType_All') is not null drop table #temp_IndexOfSpeedToExplain_QType_All
	SELECT	 [id] as [QuestionTypeId],
			 [name] as [QuestionTypeName]
	into #temp_IndexOfSpeedToExplain_QType_All
			 FROM ( select * from (
				 select  t0.id,
						t0.name,
						t1.RDAId,
	...
	--IndexOfSpeedToExplain_Place_All
	if object_id('tempdb..#temp_IndexOfSpeedToExplain_Place_All') is not null drop table #temp_IndexOfSpeedToExplain_Place_All
	SELECT	 [id] as [QuestionTypeId],
			 --[name] as [QuestionTypeName],
			 [2000],[2001],[2002],[2003],[2004],[2005],[2006],[2007],[2008],[2009]
	into #temp_IndexOfSpeedToExplain_Place_All
			 FROM ( select * from (
				 select  t0.id,
						t0.name,
						t1.RDAId,
	...
	--IndexOfSpeedToExplain_Percent
		if object_id('tempdb..#temp_IndexOfSpeedToExplain_Percent_All') is not null drop table #temp_IndexOfSpeedToExplain_Percent_All
		SELECT	 [id] as [QuestionTypeId],
			 --[name] as [QuestionTypeName],
			 [2000],[2001],[2002],[2003],[2004],[2005],[2006],[2007],[2008],[2009]
			 into #temp_IndexOfSpeedToExplain_Percent_All
			 FROM ( select * from (
				 select  t0.id,
						t0.name,
						t1.RDAId,
	 ...
	 t0.QuestionTypeId, 
	 t0.QuestionTypeName, 
	 t1.EtalonDaysToExplain as [EtalonDays],
	 ...
	 from #temp_IndexOfSpeedToExplain_QType_All as t0
...
begin
	declare @sql nvarchar(max)  = N'	
			/*if object_id(''tempdb..##temp_IndexOfSpeedToExplain'') is not null drop table ##temp_IndexOfSpeedToExplain*/
			...
			/*into ##temp_IndexOfSpeedToExplain*/
			from (
	...
	/*
	select *	
	from ##temp_IndexOfSpeedToExplain*/
end
